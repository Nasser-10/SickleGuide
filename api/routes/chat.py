...
def chat(request: ChatRequest):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"SickleGuide failed to process the request: {type(exc).__name__}: {exc}",
        ) from exc
...
async def chat_stream(request: ChatRequest):
    ...
    async def event_stream():
        try:
            ...
            # The RAG engine currently returns a verified final answer as one value.
            # Stream that verified answer word-by-word to provide a natural ChatGPT-like UI.
