...
def evaluate_retrieval(engine, cases: List[EvaluationCase] | None = None) -> Dict:
    ...
    for index, case in enumerate(cases, start=1):
        ...
        reranked = engine.reranker.rerank(query=case.query, documents=candidates, top_k=engine.final_k)

        candidate_precision_5 = precision_at_k(candidates, case.expected_sources, case.expected_pages, case.expected_keywords, k=5)
        reranked_precision_5 = precision_at_k(reranked, case.expected_sources, case.expected_pages, case.expected_keywords, k=5)
        candidate_recall_5 = recall_at_k(candidates, case.expected_sources, case.expected_pages, case.expected_keywords, k=5)
        candidate_recall_10 = recall_at_k(candidates, case.expected_sources, case.expected_pages, case.expected_keywords, k=10)
        candidate_recall_20 = recall_at_k(candidates, case.expected_sources, case.expected_pages, case.expected_keywords, k=20)
        reranked_recall_5 = recall_at_k(reranked, case.expected_sources, case.expected_pages, case.expected_keywords, k=5)
        reranked_mrr = reciprocal_rank(reranked, case.expected_sources, case.expected_pages, case.expected_keywords)
        source_recall_10 = source_recall_at_k(candidates, case.expected_sources, k=10)

        result = {
            "query": case.query,
            "precision@5": candidate_precision_5,
            "candidate_precision@5": candidate_precision_5,
        ...
        print(f"Recall@10            : {candidate_recall_10:.3f}", flush=True)
        print(f"Recall@20            : {candidate_recall_20:.3f}", flush=True)
        print(f"Rerank Recall@5      : {reranked_recall_5:.3f}", flush=True)
        print(f"MRR                  : {reranked_mrr:.3f}", flush=True)

    summary = {
        "cases": len(results),
        "candidate_precision@5": mean(item["candidate_precision@5"] for item in results),
        "reranked_precision@5": mean(item["reranked_precision@5"] for item in results),
        "precision@5": mean(item["precision@5"] for item in results),
...
def run_evaluation(run_end_to_end: bool = True) -> Dict:
    print("=" * 70, flush=True)
